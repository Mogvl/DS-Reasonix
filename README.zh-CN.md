...
> 本仓库附带完整的 Docker Compose 部署配置，适用于**绿联 NAS（UGREEN）**等
> Docker 主机：前端 **7551** 端口（nginx），后端 **7552** 端口（reasonix serve）。

```bash
cp .env.docker.example .env        # 填写 DEEPSEEK_API_KEY
docker compose up -d               # 拉取预构建镜像（amd64 + arm64）
# 国内网络：docker compose up -d --build（配合 GOPROXY=https://goproxy.cn,direct）
```

启动后浏览器访问 `http://NAS地址:7551`。详细说明见
