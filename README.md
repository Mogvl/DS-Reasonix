...
## Docker deploy (NAS / one-click)

This repository ships a complete Docker Compose setup for NAS devices (e.g.
UGREEN) and any Docker host: frontend on port **7551** (nginx), backend on port
**7552** (`reasonix serve`).

```bash
cp .env.docker.example .env        # fill in DEEPSEEK_API_KEY
docker compose up -d               # pull prebuilt multi-arch image (amd64 + arm64)
# If GHCR is unreachable: docker compose up -d --build (optionally GOPROXY=https://goproxy.cn,direct)
```

Open `http://NAS-address:7551` in your browser. See
**[docs/DEPLOY_DOCKER.md](./docs/DEPLOY_DOCKER.md)** for details.
